...
def upsert_shipping(vendor_key: str, vendor_name: str, flat_rate: float = None,
    ...
    conn.execute("""
        ...
        ON CONFLICT(vendor_key) DO UPDATE SET
            ...
            origin_zip     = COALESCE(excluded.origin_zip, vendor_shipping.origin_zip),
            carrier        = COALESCE(excluded.carrier, vendor_shipping.carrier),
            service        = COALESCE(excluded.service, vendor_shipping.service),
            flat_rate      = COALESCE(excluded.flat_rate, vendor_shipping.flat_rate),
            free_threshold = COALESCE(excluded.free_threshold, vendor_shipping.free_threshold),
            min_order      = COALESCE(excluded.min_order, vendor_shipping.min_order),
            heat_cold_pack = COALESCE(excluded.heat_cold_pack, vendor_shipping.heat_cold_pack),
            live_guarantee = excluded.live_guarantee,
            notes          = COALESCE(excluded.notes, vendor_shipping.notes),
            source_url     = COALESCE(excluded.source_url, vendor_shipping.source_url),
            scraped_at     = datetime('now')
    """, (vendor_key, vendor_name, origin_zip, carrier, service,
          flat_rate, free_threshold, min_order, heat_cold_pack,
          live_guarantee, notes, source_url))
